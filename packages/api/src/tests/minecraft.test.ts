...
describe("/games/state", () => {
  it("should get the game state", async () => {
    const res = await app.request(`${BASE_PATH}/games/state`);
  ...
  it("should get the game state with a specific gameId", async () => {
    const res = await app.request(`${BASE_PATH}/games/state?gameId=1`);
    const data = await res.json();
  ...
  it("should return 400 for non-numeric gameId", async () => {
    const res = await app.request(`${BASE_PATH}/games/state?gameId=abc`);
